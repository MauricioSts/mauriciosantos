...
function ElectricBorder({ children, color = '#10b981', speed = 1, chaos = 1, thickness = 2, className, style }) {
  ...
  const strokeStyle = { ...inheritRadius, borderWidth: thickness, borderStyle: 'solid', borderColor: color }
  const glow1Style = { ...inheritRadius, borderWidth: thickness, borderStyle: 'solid', borderColor: hexToRgba(color, 0.6), filter: `blur(${0.5 + thickness * 0.25}px)`, opacity: 0.5 }
  const glow2Style = { ...inheritRadius, borderWidth: thickness, borderStyle: 'solid', borderColor: color, filter: `blur(${2 + thickness * 0.5}px)`, opacity: 0.5 }
  const bgGlowStyle = { ...inheritRadius, transform: 'scale(1.08)', filter: 'blur(32px)', opacity: 0.3, zIndex: -1, background: `linear-gradient(-30deg, ${hexToRgba(color, 0.8)}, transparent, ${color})` }

  return (
    <div ref={rootRef} className={'relative isolate ' + (className ?? '')} style={{ ...style, backgroundColor: 'transparent' }}>
